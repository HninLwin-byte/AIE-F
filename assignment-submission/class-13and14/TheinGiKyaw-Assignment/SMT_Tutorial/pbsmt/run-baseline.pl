...
my $smtpath = dirname(abs_path($0));
my @configs = `find $smtpath/baseline/ -name "config.baseline*" | sort`;
die "No generated baseline configs found. Run: perl ./generate_configs.pl\n" unless @configs;

my @required_tools = qw(
    /home/gi/tool/giza-pp/bin-smt/mkcls
    /home/gi/tools/kenlm/build/bin/lmplz
    /home/gi/tools/kenlm/build/bin/build_binary
);

foreach my $tool (@required_tools) {
    die "Required tool is missing or not executable: $tool\nRun: ls -l $tool && ldd $tool\n" unless -x $tool;
}
...
#	chomp $b_config;
	...
	`/home/gi/tools/mosesbin/ubuntu-17.04/mosesdecoder/scripts/ems/experiment.perl -config $config -exec -no-graph 2>&1 | tee -a run1.log`;
